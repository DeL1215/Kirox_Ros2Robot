        ...
        async with self._send_lock:
            try:
                if image_b64:
                    ...
                    self.node.get_logger().info("[send_round] sent image (base64-jpeg)")
                await self._ws.send(wav_bytes)
                self.node.get_logger().info(f"[send_round] sent audio bytes: {len(wav_bytes)}")
                await self._ws.send(json.dumps({"end": True}))  # 關鍵：server 需要 end:true
                self.node.get_logger().info("[send_round] sent end:true")
                return True
            except Exception as e:
